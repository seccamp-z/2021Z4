variable "image_ubuntu_2004lts" {
  default = {
    ap-northeast-1 = "ami-0827d8ed0295e3feb"
  }
}

variable "student_count" {
  default = 4
}

provider "aws" {
  region = "ap-northeast-1"
}

resource "aws_key_pair" "seccamp_sshkey1" {
  key_name   = "seccamp_sshkey1"
  public_key = "ssh-rsa  h-shirokura@security-camp2021.jp"
}

resource "aws_vpc" "seccamp_vpc1" {
  cidr_block           = "10.255.0.0/16"
  instance_tenancy     = "default"
  enable_dns_support   = "true"
  enable_dns_hostnames = "false"

  tags = {
    Name = "seccamp_vpc1"
  }
}

resource "aws_internet_gateway" "seccamp_vpc1_igw" {
  vpc_id = "${aws_vpc.seccamp_vpc1.id}"

  tags = {
    Name = "seccamp_vpc1_igw"
  }
}

resource "aws_subnet" "seccamp_vpc1_subnet1" {
  vpc_id            = "${aws_vpc.seccamp_vpc1.id}"
  cidr_block        = "10.255.1.0/24"
  availability_zone = "ap-northeast-1a"
  map_public_ip_on_launch = "false"

  tags = {
    Name = "seccamp_vpc1_subnet1"
  }
}

resource "aws_route_table" "route_table1" {
  vpc_id = "${aws_vpc.seccamp_vpc1.id}"
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = "${aws_internet_gateway.seccamp_vpc1_igw.id}"
  }
}

resource "aws_route_table_association" "route_association1" {
  subnet_id = "${aws_subnet.seccamp_vpc1_subnet1.id}"
  route_table_id = "${aws_route_table.route_table1.id}"
}

resource "aws_security_group" "seccamp_sg1" {
  vpc_id = "${aws_vpc.seccamp_vpc1.id}"
  name   = "seccamp_sg1"

  ingress {
    description = "SSH from Any"
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  }

  ingress {
    description = "ICMP from Any"
    from_port   = -1
    to_port     = -1
    protocol    = "icmp"
    cidr_blocks = ["0.0.0.0/0"]
  }

  egress {
    description = "Any to Any"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
  }

  tags = {
    Name = "seccamp_sg1"
  }
}

resource "aws_launch_template" "seccamp_instance_template1" {
  name          = "seccamp_instance_template1"
  image_id      = "${var.image_ubuntu_2004lts.ap-northeast-1}"
  instance_type = "t2.micro"
  key_name      = "seccamp_sshkey1"

  network_interfaces {
    associate_public_ip_address = "false"
    subnet_id = "${aws_subnet.seccamp_vpc1_subnet1.id}"
    security_groups = [
      "${aws_security_group.seccamp_sg1.id}"
    ]
  }
}

resource "aws_instance" "seccamp_instance_student" {
  count = "${var.student_count}"
  launch_template { id = "${aws_launch_template.seccamp_instance_template1.id}" }
  tags = { Name = "${format("student%02d", count.index + 1)}" }
}

resource "aws_eip" "seccamp_instance_student" {
  count = "${var.student_count}"
  instance = "${element(aws_instance.seccamp_instance_student.*.id, count.index)}"
  vpc      = true
}

output "student-servers" {
  value = "${aws_instance.seccamp_instance_student.*.public_ip}"
}
